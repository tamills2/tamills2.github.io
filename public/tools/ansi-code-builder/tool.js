...
(() => {
  ...
  function updateColorOutputs(){const {rgb,hex}=colorData(),n256=nearest256(rgb),n16=nearest16(rgb),body=currentColorBody(),seq=escapeText(body,"hex"),grid=$("#color-output-grid");$("#color-swatch").style.background=hex;$("#swatch-label").textContent=hex;$("#hex-input").value=hex;[$("#red-input").value,$("#green-input").value,$("#blue-input").value]=rgb;const preview=$("#terminal-preview"),apply=$("input[name=apply-to]:checked").value,styles=$$(".style-option:checked").map(i=>i.value);const themeStyles=getComputedStyle(document.documentElement),isDark=document.documentElement.dataset.theme==="dark",themeSurface=isDark?(themeStyles.getPropertyValue("--surface-subtle").trim()||"#11151c"):"transparent",themeText=themeStyles.getPropertyValue("--text").trim()||"#1f2328";preview.style.color=apply==="background"?themeText:hex;preview.style.background=apply==="foreground"?themeSurface:apply==="background"?hex:($("#background-hex").value||themeSurface);preview.style.fontWeight=styles.includes("1")?"800":"400";preview.style.fontStyle=styles.includes("3")?"italic":"normal";preview.style.textDecoration=[styles.includes("4")?"underline":"",styles.includes("9")?"line-through":""].filter(Boolean).join(" ")||"none";grid.replaceChildren(outputItem("Hex",hex),outputItem("RGB",`rgb(${rgb.join(", ")})`),outputItem("Selected ANSI sequence",seq),outputItem("ANSI 256 approximation",`${escapeText(`[38;5;${n256.index}m`,"hex")} — index ${n256.index}`),outputItem("Standard 16-color approximation",`${n16.name} — ${escapeText(`[${n16.index<8?30+n16.index:90+n16.index-8}m`,"hex")}`),outputItem("Reset",escapeText("[0m","hex")))}
  ...
  window.addEventListener("repo-theme-change",updateColorOutputs);
