...
/* Adaptive ambient-effect quality -----------------------------------------
   Keep the full theme environments on capable systems, but automatically use
   a compositor-light version on machines that report limited memory/CPU or a
   very large high-DPI viewport. This is deliberately visual-only: theme
   colors, fonts, layout, and controls are unchanged. */
(() => {
  const root = document.documentElement;
  const cores = Number(navigator.hardwareConcurrency || 0);
  const memory = Number(navigator.deviceMemory || 0);
  const pixelLoad = window.innerWidth * window.innerHeight * Math.max(1, window.devicePixelRatio || 1) ** 2;

  const limitedCpu = cores > 0 && cores <= 4;
  const limitedMemory = memory > 0 && memory <= 4;
  const highPixelLoad = pixelLoad >= 5_000_000;
  const prefersReducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

  root.classList.toggle(
    "theme-effects-lite",
    prefersReducedMotion || limitedCpu || limitedMemory || highPixelLoad
  );
})();
